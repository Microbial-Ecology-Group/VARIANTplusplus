...
- Creates 3 bash scripts:
  1) script_build_reads_{PREFIX}.sh
       * For each (numGSV, iteration):
         - If numGSV==0 => ONE off-target sample per iteration:
           pick non-target genomes => cat references => ISS => FLASH => rename
         - If numGSV>0 => for each k_col:
           pick multiple GSVs => cat references => ISS => FLASH => rename
  ...
  3) script_themisto_msweep_{PREFIX}.sh
       ...
       * Off-target samples are classified against ALL k_col annotations.
       * On-target samples are classified against their matching k_col only.
"""
...
def write_simulate_and_rename(build_out, tmp_build, bin_dir, results_dir,
                               threads, iss_prefix, partial_name,
                               tmp_cat, read_count):
    """
    Write bash commands for shared simulation pipeline:
      ISS generate → FLASH merge → cat merged+unmerged →
      rename_sample_files.py → rename_headers.py → cleanup

    Returns the renamed partial filename (basename only).
    """
    nodir_prefix = os.path.basename(iss_prefix)

    build_out.write(
        f"iss generate --genomes {tmp_cat} --n_reads {read_count} --model NovaSeq "
        f"--cpus {threads} --output {iss_prefix} --compress --quiet "
        f"--fragment-length 311 --fragment-length-sd 50\n"
    )
    build_out.write(
    ...
    # Embed sample identity into read headers
...
def main():
    ...
    offtarget_iter_multiplier = params.get("offtarget_iter_multiplier", 1)
    ...
    # Create mSWEEP annotation files
    ...
    with open(script_build_reads, 'w') as build_out:
        ...
        for numGSV in num_GSV_list:
            ...
            effective_iters = num_iters * offtarget_iter_multiplier if numGSV == 0 else num_iters
            ...
            for iteration in range(1, effective_iters + 1):
                ...
                if numGSV == 0:
                    # ----------------------------------------------------------
                    # Off-target: ONE sample per iteration (no k_col loop).
                    # Non-target genomes don't have GSV annotations, so k_col
                    # is irrelevant for genome selection. The sample will be
                    # classified against ALL k_col annotations in the mSWEEP step.
                    # ----------------------------------------------------------
                    build_out.write(f"\n# (off-target sample — no k_col loop)\n")
                    ...
                    tmp_cat = f"{tmp_build}/iter_{iteration}_0GSVs.fna"
                    ...
                    iss_prefix = f"{tmp_build}/iter_{iteration}XX{numGSV}_GSVs_iss"
                    partial_name = f"{iteration}XX{numGSV}_GSVsXXoff_target"

                    renamed_part = write_simulate_and_rename(
                        build_out, tmp_build, bin_dir, results_dir, threads,
                        iss_prefix, partial_name, tmp_cat, read_count
                    )
                    ...
                    # ----------------------------------------------------------
                    # On-target: one sample per k_col
                    # ----------------------------------------------------------
                ...
                # Merge partial samples across k_col => iteration-level final
        ...
        for partial_name, col_info in sample_map.items():
            for conf in kraken_confidence:
                ...
                if col_info == "off_target":
                    # Off-target: classify against ALL annotation clusters
                    ...
                    # On-target: classify against matching k_col only
