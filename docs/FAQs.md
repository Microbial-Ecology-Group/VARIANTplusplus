...
* Are you using the correct "profile" to run VARIANT++?
  * We provide many examples of profile configuration and choosing the correct one depends on your computing environment.
    * We recommend the "conda" profile, which builds a conda environment with all required tools — see [installation.md](installation.md).
    * If you already have the tools installed on your server, the best option is to configure the local.config file to point to the absolute PATH to each tool.
